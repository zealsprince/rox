...
| Sharing   | Discord rich presence whose two card lines are patterns over the playing track, with optional Last.fm and YouTube buttons and a pick of which line shows beside your name in the member list, and an Icecast source client that pushes what rox plays out as MP3, connecting outward only so an unreachable server never touches local playback |
