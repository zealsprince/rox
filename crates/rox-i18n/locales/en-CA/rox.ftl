...
library-empty-note = It gets scanned into the library (flac, mp3, ogg, opus, m4a, aac, wav, aiff)
...
settings-integrations-discord-first-line = First Line
    .description = The card's first line, under rox, and what the member list repeats by default
settings-integrations-discord-hover-line = Artwork Tooltip
    .description = The text behind the cover art, on hover. Empty leaves the art without a tooltip
settings-integrations-discord-line-off = Empty, so the card leaves this line off
settings-integrations-discord-line-preview = Preview: { $line }
settings-integrations-discord-second-line = Second Line
    .description = The card's second line. Leave it empty to drop it from the card
...
settings-integrations-discord-status-line = Status Line
    .description = What Discord shows beside your name in the member list
settings-integrations-discord-status-line-app = App Name
settings-integrations-discord-status-line-first = First Line
settings-integrations-discord-status-line-second = Second Line
...
settings-integrations-subsonic-name = Name
    .description = What rox calls this server; its address stands in while this is empty
settings-integrations-subsonic-name-placeholder = Home
settings-integrations-subsonic-new = New Server
settings-integrations-subsonic-password-placeholder = Password
settings-integrations-subsonic-remove = Remove Server
    .description = Forget this server's login and delete its tracks from the library; switching it off keeps them, hidden
...
settings-integrations-subsonic-use = Use This Server
    .description = Read its library and play from it; off keeps its tracks in the library, hidden
...
settings-library-add-folder = Folder...
...
settings-playback-live-buffer-cap = The buffer stops at { $cap } however long it's set: a tenth of this machine's memory.
...
settings-playback-capture-buffer-title = The live buffer is shorter than most songs
settings-playback-capture-buffer-note = It's { $buffer }, and a song only gets saved if the whole of it is still in the buffer when it ends. { $default } covers nearly anything a station plays.
...
settings-confirm-remove-subsonic-body = Its login is forgotten and its tracks leave the library. Having them back means connecting and syncing again; switching it off keeps them instead.
settings-confirm-remove-subsonic-title = Remove "{ $name }"?
...
tags-editor-guess-help = / matches the folder above, %skip% discards
...
tags-guess-placeholders = Placeholders
...
tags-rename-pattern-help = / makes a folder from the library root, no / renames in place, the extension follows the file
...
convert-dialog-pattern-help = / makes a folder, the format sets the extension
...
lyrics-bold = Bold
lyrics-build-word-by-word = Follow Words as Sung
    .description = Run a read head through the active line, off the sheet's own word timings when it has them
...
lyrics-head-build = Build
lyrics-head-fill = Fill
lyrics-hide-upcoming = Hide Upcoming Lines
    .description = Keep every line the playhead hasn't reached out of sight, so the sheet reveals itself as it plays
...
lyrics-read-head-style = Read Head
    .description = How the active line shows the head: a karaoke fill sweeping across it, or each word building in as its turn comes
...
lyrics-unsung-dim = Unsung Dim
    .description = How far the fill dims the part of the active line that hasn't been sung yet
lyrics-wipe-lyrics = Wipe Lyrics
lyrics-wrap-lines = Wrap Long Lines
    .description = Let a line too wide for the panel run onto another row instead of being cut off
