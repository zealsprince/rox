...
stations-checking = ストリームを確認中...
stations-is-hls = これは HLS のセグメントプレイリストです。rox が再生できるのは連続したストリームです
stations-is-a-playlist = これはラジオ局の一覧です。ラジオ局をインポートから読み込んでください
stations-not-audio = この URL は { $kind } を返します。音声ではありません
stations-added-unchecked = 確認せずに追加しました: ストリームが応答しませんでした ({ $reason })
...
settings-playback-capture-pattern-source = %source% はストリームの取得元の種類です。新しい種類の取得元は専用のフォルダーに保存されます。
...
settings-playback-section-streaming = ストリーミング
...
autoeq-save = 保存
autoeq-saved = 保存済み
autoeq-saved-info = 「{ $name }」として保存しました
autoeq-save-failed = プリセットファイルを保存できませんでした
autoeq-applied-info = 適用しました: { $name } ({ $source })
autoeq-applied-info-preamp = 適用しました: { $name } ({ $source })、プリアンプ { $db } dB
autoeq-imported-info = インポートしました: { $name }
autoeq-imported-info-preamp = インポートしました: { $name }、プリアンプ { $db } dB
autoeq-read-failed = ファイルを開けませんでした: { $reason }
autoeq-parse-failed = プロファイルを読み取れませんでした: { $reason }
autoeq-import-untitled = カスタム
eq-flatten = フラットにする
eq-freq-label = 周波数
eq-gain-label = ゲイン
eq-heading = イコライザー
...
eq-open = イコライザーを開く
eq-preset-export = エクスポート
eq-preset-export-default = イコライザー
eq-preset-name = プリセット名
eq-preset-save = プリセットを保存
eq-presets = プリセット
...
keymap-cue-clear = マーカーを消去
    .description = 再生中のトラックのセッションマーカーをすべて削除する
...
lastfm-import-history = スクロブル履歴を読み込み中
...
player-stream-refused = トラックをスキップしました: { $reason }
