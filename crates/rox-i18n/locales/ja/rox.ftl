...
library-empty-note = ライブラリに読み込まれます (flac, mp3, ogg, opus, m4a, aac, wav, aiff)
library-headers = 見出し
...
settings-appearance-chrome-side = ウィンドウボタンの位置
    .description = rox 自前のタイトルバーでボタンを置く側。コンポジターがウィンドウを装飾しない環境と、枠なしの子ウィンドウで描かれる
settings-appearance-chrome-side-left = 左
settings-appearance-chrome-side-right = 右
settings-appearance-chrome-style = ウィンドウボタンのスタイル
    .description = rox 自前のタイトルバーのボタンの描き方。コンポジターがウィンドウを装飾しない環境と、枠なしの子ウィンドウで描かれる
settings-appearance-cjk-fonts-note = ライブラリの日本語、中国語、韓国語のテキストは、インストール済みのフォントを一つずつ試して表示されます。そのためスクロールが重くなり、代わりのフォントで描かれます。静的な Noto Sans CJK フォントを入れてください。NixOS では noto-fonts-cjk-sans-static、Fedora では google-noto-sans-cjk-fonts、Arch では noto-fonts-cjk、Debian では fonts-noto-cjk です。
settings-appearance-cjk-fonts-title = 標準ウェイトの CJK フォントがありません
...
settings-appearance-bare-child-windows = 子ウィンドウも枠なしにする
    .description = OS のウィンドウ枠がオフのとき、設定、エディター、切り離したパネルも OS の枠を外し、rox 自前のタイトルバーを描く。リサイズ用の縁もこれらに効く
settings-appearance-child-titlebar = 子ウィンドウのタイトルバー
    .description = 枠なしの子ウィンドウに rox が描くタイトルバー。オフにすると枠が何もなくなり、「ウィンドウを閉じる」のショートカットで閉じる
...
settings-integrations-discord-first-line = 1 行目
    .description = カードの rox の下に来る 1 行目で、既定ではメンバーリストがこれを繰り返す
settings-integrations-discord-hover-line = アートワークのツールチップ
    .description = カバーアートにポインターを乗せたときに出る文字。空ならツールチップは出ない
settings-integrations-discord-line-off = 空なので、カードはこの行を省く
settings-integrations-discord-line-preview = プレビュー: { $line }
settings-integrations-discord-second-line = 2 行目
    .description = カードの 2 行目。空にするとカードから外れる
...
settings-integrations-discord-status-line = ステータス行
    .description = メンバーリストで名前の横に Discord が表示する内容
settings-integrations-discord-status-line-app = アプリ名
settings-integrations-discord-status-line-first = 1 行目
settings-integrations-discord-status-line-second = 2 行目
...
settings-integrations-subsonic-name = 名前
    .description = rox がこのサーバーを呼ぶ名前。空のあいだはアドレスが代わりに表示されます
settings-integrations-subsonic-name-placeholder = 自宅
settings-integrations-subsonic-new = 新しいサーバー
settings-integrations-subsonic-password-placeholder = パスワード
settings-integrations-subsonic-remove = サーバーを削除
    .description = このサーバーのログインを消し、その曲をライブラリーから削除します。オフにした場合、曲は隠れるだけで残ります
...
settings-integrations-subsonic-use = このサーバーを使う
    .description = ライブラリーを読み込んで再生します。オフにすると曲はライブラリーに残り、隠れます
...
settings-library-add-folder = フォルダー...
settings-library-add-subsonic = Subsonic サーバー...
settings-library-col-source = ソース
settings-library-duplicates = 重複...
settings-library-embed-button = 保存済みメタデータを埋め込む...
settings-library-exclude-col = 除外
settings-library-exclude-intro = 上のすべてのソースでライブラリが読み込まないファイルとフォルダーです。*.tmp のような名前はどこでも一致し、Live/** のようなパスは各フォルダーの最上位から数えます。大文字と小文字は区別せず、一致したフォルダーは中身ごと飛ばします。ファイル自体はそのまま残ります
settings-library-exclude-invalid = このパターンは読み取れません: { $reason }
settings-library-exclude-none = 除外なし
settings-library-exclude-nudge = パターンが変わりました。監視にはすぐ効きます。すでにライブラリにあるトラックは、上のソース見出しから再スキャンするまで残ります
settings-library-exclude-placeholder = *.tmp または Live/**
settings-library-folder-col-albums = アルバム
settings-library-folder-col-size = サイズ
settings-library-folder-col-tracks = トラック
settings-library-folders-intro = ライブラリの読み込み元になるフォルダーとサーバーです。外すとそのトラックはカタログから消えますが、フォルダーのファイルはそのまま残ります
settings-library-genre-separator-nudge = 区切り文字が変わりました。表示にはすぐ効きます。以前のスキャンで保存されたジャンルのリストは、上のソース見出しから再スキャンするまで古い形のままです
...
settings-library-no-sources = ソースがまだありません
...
settings-library-section-sources = ソース
...
settings-playback-live-buffer-cap = 設定の長さに関わらずバッファは { $cap } で止まる。このマシンのメモリの10分の1。
...
settings-playback-capture-buffer-title = ライブバッファがたいていの曲より短い
settings-playback-capture-buffer-note = いまは { $buffer }。曲は終わった時点で丸ごとバッファに残っていないと保存されない。{ $default } あれば局がかけるものはほぼ収まる。
...
settings-confirm-remove-subsonic-body = ログインは消え、このサーバーの曲はライブラリーから外れます。戻すには接続と同期をやり直すことになります。オフにすれば曲は残ります。
settings-confirm-remove-subsonic-title = "{ $name }" を削除しますか?
...
tags-editor-guess-help = / は一つ上のフォルダーに対応し、%skip% は読み捨てる
...
tags-guess-placeholders = プレースホルダー
...
tags-rename-pattern-help = / はライブラリのルートからフォルダーを作り、/ がなければその場で名前を変え、拡張子はファイルのものが付く
...
convert-dialog-pattern-help = / はフォルダーを作り、拡張子は形式が決める
...
lyrics-bold = 太字
lyrics-build-word-by-word = 歌詞を歌に合わせて追う
    .description = 現在行に読み頭を走らせる。詞ごとの時刻を持つ歌詞ならそれに従う
...
lyrics-head-build = 組み上げ
lyrics-head-fill = 塗りつぶし
lyrics-hide-upcoming = これからの行を隠す
    .description = 再生位置がまだ届いていない行を見せずにおき、再生につれて歌詞が現れるようにする
...
lyrics-read-head-style = 読み頭
    .description = 現在行の読み頭の見せ方。行を横切るカラオケの塗りつぶしか、順番に組み上がる単語か
...
lyrics-unsung-dim = 未歌部分の暗さ
    .description = 塗りつぶしが、現在行のまだ歌われていない部分をどれくらい暗くするか
lyrics-wipe-lyrics = 歌詞を消す
lyrics-wrap-lines = 長い行を折り返す
    .description = パネルに入りきらない行を切り捨てず、次の行に送る
