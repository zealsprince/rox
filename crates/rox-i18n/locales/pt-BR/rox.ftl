...
library-empty-note = Ela é escaneada para a biblioteca (flac, mp3, ogg, opus, m4a, aac, wav, aiff)
...
stations-checking = Verificando o fluxo...
stations-is-hls = Isso é uma lista de segmentos HLS; o rox toca um fluxo contínuo
stations-is-a-playlist = Isso é uma lista de estações; leia com Importar estações
stations-not-audio = Essa URL responde com { $kind }, não com áudio
stations-added-unchecked = Adicionada sem verificação: o fluxo não respondeu ({ $reason })
...
settings-appearance-chrome-side = Lado dos botões da janela
    .description = Em que ponta da barra de título própria do rox os botões ficam; desenhada onde o compositor não decora a janela e nas janelas secundárias sem moldura
settings-appearance-chrome-side-left = Esquerda
settings-appearance-chrome-side-right = Direita
settings-appearance-chrome-style = Estilo dos botões da janela
    .description = Como os botões da barra de título própria do rox são desenhados; desenhada onde o compositor não decora a janela e nas janelas secundárias sem moldura
settings-appearance-cjk-fonts-note = O texto em japonês, chinês ou coreano da sua biblioteca tenta cada fonte instalada, uma por uma. Isso deixa a rolagem lenta e desenha o texto numa fonte substituta. Instale as fontes Noto Sans CJK estáticas: noto-fonts-cjk-sans-static no NixOS, google-noto-sans-cjk-fonts no Fedora, noto-fonts-cjk no Arch, fonts-noto-cjk no Debian.
settings-appearance-cjk-fonts-title = Nenhuma fonte CJK de peso normal
...
settings-appearance-bare-child-windows = Janelas secundárias sem moldura
    .description = Com as Decorações do sistema desligadas, as configurações, os editores e os painéis destacados também perdem a moldura do sistema e desenham a barra de título própria do rox; a Borda de redimensionamento vale para elas também
settings-appearance-child-titlebar = Barra de título das janelas secundárias
    .description = A barra de título que o rox desenha nas janelas secundárias sem moldura; desligada, elas ficam sem nada em volta e o atalho Fechar janela as fecha
...
settings-integrations-discord-first-line = Primeira linha
    .description = A primeira linha do cartão, abaixo do rox, e o que a lista de membros repete por padrão
settings-integrations-discord-hover-line = Tooltip da capa
    .description = O texto atrás da capa, ao passar o ponteiro. Vazio deixa a capa sem tooltip
settings-integrations-discord-line-off = Vazia, então o cartão deixa esta linha de fora
settings-integrations-discord-line-preview = Prévia: { $line }
settings-integrations-discord-second-line = Segunda linha
    .description = A segunda linha do cartão. Deixe vazia para tirá-la do cartão
...
settings-integrations-discord-status-line = Linha de status
    .description = O que o Discord mostra ao lado do seu nome na lista de membros
settings-integrations-discord-status-line-app = Nome do app
settings-integrations-discord-status-line-first = Primeira linha
settings-integrations-discord-status-line-second = Segunda linha
...
settings-integrations-subsonic-name = Nome
    .description = Como o rox chama este servidor; enquanto estiver vazio, o endereço fica no lugar
settings-integrations-subsonic-name-placeholder = Casa
settings-integrations-subsonic-new = Novo servidor
settings-integrations-subsonic-password-placeholder = Senha
settings-integrations-subsonic-remove = Remover servidor
    .description = Esquece o acesso a este servidor e apaga as faixas dele da biblioteca; desativá-lo as mantém, ocultas
...
settings-integrations-subsonic-use = Usar este servidor
    .description = Lê a biblioteca dele e toca a partir dela; desativado, as faixas dele ficam na biblioteca, ocultas
...
settings-library-add-folder = Pasta...
settings-library-add-subsonic = Servidor Subsonic...
settings-library-col-source = Fonte
settings-library-duplicates = Duplicatas...
settings-library-embed-button = Incorporar metadados salvos...
settings-library-exclude-col = Excluídos
settings-library-exclude-intro = Arquivos e pastas que a biblioteca deixa de fora de todas as fontes acima. Um nome como *.tmp vale em qualquer lugar, e um caminho como Live/** começa no topo de cada pasta. Maiúsculas e minúsculas não importam, e uma pasta que bate é pulada inteira. Os arquivos em si ficam onde estão
settings-library-exclude-invalid = Não dá para ler esse padrão: { $reason }
settings-library-exclude-none = Nada excluído
settings-library-exclude-nudge = Padrões mudaram: a vigilância acompanha na hora. As faixas que já estão na biblioteca ficam até você apertar Reescanear no cabeçalho Fontes lá em cima
...
settings-library-folders-intro = Pastas e servidores de onde a biblioteca é lida; remover um tira as faixas dele do catálogo e deixa em paz os arquivos de uma pasta
settings-library-genre-separator-nudge = Separadores mudaram: a navegação acompanha na hora. As listas de gênero guardadas por varreduras anteriores mantêm a forma antiga até você apertar Reescanear no cabeçalho Fontes lá em cima
...
settings-library-no-sources = Nenhuma fonte ainda
...
settings-library-section-sources = Fontes
...
settings-library-subsonic-add-note = Funciona com Navidrome, Airsonic, gonic e qualquer outro servidor que fale a API do Subsonic
settings-library-subsonic-add-title = Adicionar um servidor Subsonic
...
settings-playback-live-buffer-cap = O buffer para em { $cap } por mais longo que seja o ajuste: um décimo da memória desta máquina.
...
settings-playback-capture-buffer-title = O buffer do ao vivo é mais curto que a maioria das músicas
settings-playback-capture-buffer-note = Está em { $buffer }, e uma música só é salva se ainda couber inteira no buffer quando termina. { $default } cobre quase tudo que uma estação toca.
...
settings-playback-capture-pattern-source = %source% é o tipo de fonte de onde vem a transmissão, então um tipo novo vai para uma pasta própria.
...
settings-confirm-remove-subsonic-body = O acesso é esquecido e as faixas dele saem da biblioteca. Tê-las de novo significa conectar e sincronizar outra vez; desativá-lo, por outro lado, as mantém.
settings-confirm-remove-subsonic-title = Remover "{ $name }"?
...
autoeq-save = Salvar
autoeq-saved = Salvo
autoeq-saved-info = Salvo como "{ $name }"
autoeq-save-failed = Não foi possível gravar o arquivo de predefinição
autoeq-applied-info = Aplicado: { $name } ({ $source })
autoeq-applied-info-preamp = Aplicado: { $name } ({ $source }), pré-amplificação { $db } dB
autoeq-imported-info = Importado: { $name }
autoeq-imported-info-preamp = Importado: { $name }, pré-amplificação { $db } dB
autoeq-read-failed = Não foi possível abrir o arquivo: { $reason }
autoeq-parse-failed = Não foi possível ler o perfil: { $reason }
autoeq-import-untitled = Personalizado
eq-flatten = Aplanar
eq-freq-label = Freq
eq-gain-label = Ganho
...
eq-preset-export = Exportar
eq-preset-export-default = Equalizador
eq-preset-name = Nome da predefinição
eq-preset-save = Salvar predefinição
eq-presets = Predefinições
...
keymap-cue-clear = Limpar pontos
    .description = Remover todos os pontos de sessão da faixa em reprodução
...
lastfm-import-history = Lendo o histórico de scrobbles
...
tags-editor-guess-help = / casa com a pasta acima, %skip% descarta
...
tags-guess-placeholders = Marcadores
...
tags-rename-pattern-help = / cria uma pasta a partir da raiz da biblioteca, sem / renomeia no lugar, a extensão segue o arquivo
...
convert-dialog-pattern-help = / cria uma pasta, o formato define a extensão
...
lyrics-bold = Negrito
lyrics-build-word-by-word = Seguir as palavras cantadas
    .description = Correr um cursor de leitura pela linha ativa, pelos tempos de palavra da folha quando ela tem
...
lyrics-head-build = Montagem
lyrics-head-fill = Preenchimento
lyrics-hide-upcoming = Ocultar as linhas seguintes
    .description = Manter fora de vista cada linha que o cursor de reprodução ainda não alcançou, para a folha ir se revelando
...
lyrics-read-head-style = Cursor de leitura
    .description = Como a linha ativa mostra o cursor: um preenchimento de karaokê varrendo a linha, ou cada palavra se montando na sua vez
...
lyrics-unsung-dim = Escurecer o não cantado
    .description = O quanto o preenchimento escurece a parte da linha ativa que ainda não foi cantada
lyrics-wipe-lyrics = Apagar a letra
lyrics-wrap-lines = Quebrar linhas longas
    .description = Deixar uma linha larga demais para o painel seguir em outra fila em vez de ser cortada
...
player-stream-refused = Faixa ignorada: { $reason }
