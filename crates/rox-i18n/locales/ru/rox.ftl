...
library-empty-note = Она попадёт в медиатеку при сканировании (flac, mp3, ogg, opus, m4a, aac, wav, aiff)
...
stations-checking = Проверка потока...
stations-is-hls = Это список сегментов HLS; rox играет один непрерывный поток
stations-is-a-playlist = Это список станций; откройте его через Импортировать станции
stations-not-audio = Этот URL отвечает { $kind }, а не аудио
stations-added-unchecked = Добавлена без проверки: поток не ответил ({ $reason })
...
settings-appearance-chrome-side = Сторона кнопок окна
    .description = С какого края собственного заголовка rox стоят кнопки; рисуется там, где композитор не оформляет окно, и на дочерних окнах без рамки
settings-appearance-chrome-side-left = Слева
settings-appearance-chrome-side-right = Справа
settings-appearance-chrome-style = Стиль кнопок окна
    .description = Как рисуются кнопки на собственном заголовке rox; рисуется там, где композитор не оформляет окно, и на дочерних окнах без рамки
settings-appearance-cjk-fonts-note = Для японского, китайского и корейского текста в вашей библиотеке по очереди перебираются все установленные шрифты. Из-за этого прокрутка тормозит, а текст рисуется запасным шрифтом. Установите статические шрифты Noto Sans CJK: noto-fonts-cjk-sans-static в NixOS, google-noto-sans-cjk-fonts в Fedora, noto-fonts-cjk в Arch, fonts-noto-cjk в Debian.
settings-appearance-cjk-fonts-title = Нет шрифта CJK обычного начертания
...
settings-appearance-bare-child-windows = Дочерние окна без рамки
    .description = При выключенном оформлении ОС настройки, редакторы и вынесенные панели тоже теряют рамку ОС и рисуют собственный заголовок rox; рамка изменения размера действует и на них
settings-appearance-child-titlebar = Заголовок дочерних окон
    .description = Заголовок, который rox рисует на дочерних окнах без рамки; если выключить, вокруг них ничего не останется, а закрывать их будет сочетание «Закрыть окно»
...
settings-integrations-discord-first-line = Первая строка
    .description = Первая строка карточки, под rox, и то, что список участников повторяет по умолчанию
settings-integrations-discord-hover-line = Подсказка у обложки
    .description = Текст за обложкой, при наведении. Пусто — обложка без подсказки
settings-integrations-discord-line-off = Пусто, карточка обходится без этой строки
settings-integrations-discord-line-preview = Предпросмотр: { $line }
settings-integrations-discord-second-line = Вторая строка
    .description = Вторая строка карточки. Оставьте пустой, и её не будет на карточке
...
settings-integrations-discord-status-line = Строка статуса
    .description = Что Discord показывает рядом с вашим именем в списке участников
settings-integrations-discord-status-line-app = Название приложения
settings-integrations-discord-status-line-first = Первая строка
settings-integrations-discord-status-line-second = Вторая строка
...
settings-integrations-subsonic-name = Название
    .description = Как rox называет этот сервер; пока поле пустое, вместо него показывается адрес
settings-integrations-subsonic-name-placeholder = Дом
settings-integrations-subsonic-new = Новый сервер
settings-integrations-subsonic-password-placeholder = Пароль
settings-integrations-subsonic-remove = Удалить сервер
    .description = Забыть вход на этот сервер и удалить его треки из медиатеки; если выключить, треки останутся, но скрытыми
...
settings-integrations-subsonic-use = Использовать этот сервер
    .description = Читать его медиатеку и играть с него; если выключить, треки останутся в медиатеке, но скрытыми
...
settings-library-add-folder = Папка...
settings-library-add-subsonic = Сервер Subsonic...
settings-library-col-source = Источник
settings-library-duplicates = Дубликаты...
settings-library-embed-button = Встроить сохранённые метаданные...
settings-library-exclude-col = Исключено
settings-library-exclude-intro = Файлы и папки, которые медиатека пропускает во всех источниках выше. Имя вроде *.tmp совпадает где угодно, а путь вроде Live/** отсчитывается от верха каждой папки. Регистр не важен, а совпавшая папка пропускается целиком. Сами файлы никто не трогает
settings-library-exclude-invalid = Не удаётся прочитать шаблон: { $reason }
settings-library-exclude-none = Ничего не исключено
settings-library-exclude-nudge = Шаблоны изменились: слежение подхватит это сразу. Треки, которые уже в медиатеке, останутся, пока вы не нажмёте «Пересканировать» в шапке раздела «Источники»
settings-library-exclude-placeholder = *.tmp или Live/**
...
settings-library-folders-intro = Папки и серверы, из которых читается медиатека; удаление источника убирает его треки из каталога, а файлы папки не трогает
settings-library-genre-separator-nudge = Разделители изменились: просмотр подхватит это сразу. Списки жанров, сохранённые прошлыми сканированиями, останутся в прежнем виде, пока вы не нажмёте «Пересканировать» в шапке раздела «Источники»
...
settings-library-no-sources = Пока нет источников
...
settings-library-section-sources = Источники
...
settings-library-subsonic-add-note = Подходит Navidrome, Airsonic, gonic и любой другой сервер с API Subsonic
settings-library-subsonic-add-title = Добавить сервер Subsonic
...
settings-playback-live-buffer-cap = Буфер останавливается на { $cap }, какой бы длины он ни был: десятая часть памяти этой машины.
...
settings-playback-capture-buffer-title = Буфер эфира короче большинства песен
settings-playback-capture-buffer-note = Сейчас { $buffer }, а песня сохраняется, только если к её концу она целиком осталась в буфере. { $default } хватает почти на всё, что ставят станции.
...
settings-playback-capture-pattern-source = %source% обозначает тип источника, откуда идёт поток, так что новый тип источника попадает в свою папку.
...
settings-playback-section-streaming = Стриминг
...
settings-storage-listening-history = История прослушиваний
    .description = Каждое воспроизведение, о котором знает библиотека: собственные и всё, что принёс импорт из Last.fm. Отсюда берутся столбец воспроизведений, окно статистики, панель истории и любой умный плейлист, который сортирует по воспроизведениям
...
settings-confirm-remove-subsonic-body = Вход будет забыт, а треки сервера уйдут из медиатеки. Чтобы вернуть их, придётся снова подключиться и синхронизировать; выключение, наоборот, их сохраняет.
settings-confirm-remove-subsonic-title = Удалить «{ $name }»?
...
## статистики и страница хранилища, и число в их строках.

listens-clear-body = { $imported } из { $total } прослушиваний пришли из импорта Last.fm: с датой там, где её знал аккаунт, и разбросаны по годам там, где осталось только число. Очистить только их означает сохранить прослушивания, которые rox видел сам; очистить всё означает оставить библиотеку без истории.
listens-clear-body-plain = { $listens }, ни одного из импорта: rox видел каждое сам. Очистка оставит библиотеку без истории.
listens-clear-button = Очистить историю
listens-clear-everything = Очистить всё
listens-clear-imported = Очистить импортированные
listens-clear-title = Очистить историю прослушиваний?
listens-count = { $count ->
    [one] { $count } прослушивание
    [few] { $count } прослушивания
    [many] { $count } прослушиваний
...
autoeq-save = Сохранить
autoeq-saved = Сохранено
autoeq-saved-info = Сохранено как «{ $name }»
autoeq-save-failed = Не удалось записать файл пресета
autoeq-applied-info = Применено: { $name } ({ $source })
autoeq-applied-info-preamp = Применено: { $name } ({ $source }), предусиление { $db } дБ
autoeq-imported-info = Импортировано: { $name }
autoeq-imported-info-preamp = Импортировано: { $name }, предусиление { $db } дБ
autoeq-read-failed = Не удалось открыть файл: { $reason }
autoeq-parse-failed = Не удалось прочитать профиль: { $reason }
autoeq-import-untitled = Свой профиль
...
eq-preset-export = Экспорт
eq-preset-export-default = Эквалайзер
eq-preset-name = Название пресета
...
keymap-cue-clear = Очистить метки
    .description = Убрать все сеансовые метки с играющего трека
...
lastfm-import-history = Чтение истории скробблов
...
tags-editor-guess-help = / соответствует папке выше, %skip% отбрасывает
...
tags-guess-placeholders = Подстановки
...
tags-rename-pattern-help = / создаёт папку от корня библиотеки, без / переименовывает на месте, расширение берётся от файла
...
convert-dialog-pattern-help = / создаёт папку, расширение задаёт формат
...
lyrics-bold = Жирный
lyrics-build-word-by-word = Следовать за словами
    .description = Вести указатель чтения по активной строке, по собственным таймингам слов, если они есть в листе
...
lyrics-head-build = Сборка
lyrics-head-fill = Заливка
lyrics-hide-upcoming = Скрывать следующие строки
    .description = Держать скрытой каждую строку, до которой указатель ещё не дошёл, чтобы лист открывался по ходу
...
lyrics-read-head-style = Указатель чтения
    .description = Как активная строка показывает указатель: караоке-заливкой по всей строке или словами, собирающимися по очереди
...
lyrics-unsung-dim = Затемнение неспетого
    .description = Насколько заливка гасит ещё не спетую часть активной строки
lyrics-wipe-lyrics = Стереть текст
lyrics-wrap-lines = Переносить длинные строки
    .description = Пусть слишком широкая для панели строка переходит на следующую, а не обрезается
...
player-stream-refused = Трек пропущен: { $reason }
