...
library-empty-note = 它会被扫描进媒体库（flac、mp3、ogg、opus、m4a、aac、wav、aiff）
...
settings-appearance-chrome-side = 窗口按钮位置
    .description = rox 自绘标题栏上按钮靠哪一端；在合成器不给窗口加装饰时画，子窗口去掉装饰时也画
settings-appearance-chrome-side-left = 左
settings-appearance-chrome-side-right = 右
settings-appearance-chrome-style = 窗口按钮样式
    .description = rox 自绘标题栏上的按钮怎么画；在合成器不给窗口加装饰时画，子窗口去掉装饰时也画
settings-appearance-cjk-fonts-note = 曲库里的日文、中文或韩文文字会把已安装的字体挨个试一遍，所以滚动会卡，文字也会用替代字体来画。请安装静态版 Noto Sans CJK 字体：NixOS 上是 noto-fonts-cjk-sans-static，Fedora 上是 google-noto-sans-cjk-fonts，Arch 上是 noto-fonts-cjk，Debian 上是 fonts-noto-cjk。
settings-appearance-cjk-fonts-title = 没有常规字重的 CJK 字体
...
settings-appearance-bare-child-windows = 子窗口也去掉装饰
    .description = 关掉系统窗口装饰时，设置、编辑器和弹出的面板也去掉系统边框，改用 rox 自绘的标题栏；调整大小边框对它们同样生效
settings-appearance-child-titlebar = 子窗口标题栏
    .description = rox 在去掉装饰的子窗口上自绘的标题栏；关掉后窗口四周什么都没有，用“关闭窗口”快捷键关掉它们
...
settings-integrations-discord-first-line = 第一行
    .description = 卡片上 rox 下面的第一行，默认也是成员列表重复的那一行
settings-integrations-discord-hover-line = 封面提示
    .description = 指针停在封面上时显示的文字。留空就不显示提示
settings-integrations-discord-line-off = 是空的，卡片会省掉这一行
settings-integrations-discord-line-preview = 预览：{ $line }
settings-integrations-discord-second-line = 第二行
    .description = 卡片的第二行。留空就从卡片上去掉
...
settings-integrations-discord-status-line = 状态行
    .description = Discord 在成员列表中你的名字旁显示的内容
settings-integrations-discord-status-line-app = 应用名称
settings-integrations-discord-status-line-first = 第一行
settings-integrations-discord-status-line-second = 第二行
...
settings-integrations-subsonic-name = 名称
    .description = rox 对这个服务器的称呼；留空时用地址代替
settings-integrations-subsonic-name-placeholder = 家里
settings-integrations-subsonic-new = 新服务器
settings-integrations-subsonic-password-placeholder = 密码
settings-integrations-subsonic-remove = 移除服务器
    .description = 忘记这个服务器的登录，并从媒体库删除它的曲目；关闭它则保留曲目，只是隐藏
...
settings-integrations-subsonic-use = 使用这个服务器
    .description = 读取它的媒体库并播放；关闭后它的曲目仍留在媒体库里，只是隐藏
...
settings-library-add-folder = 文件夹...
settings-library-add-subsonic = Subsonic 服务器...
settings-library-col-source = 来源
settings-library-duplicates = 重复曲目…
settings-library-embed-button = 嵌入已存储的元数据…
settings-library-exclude-col = 已排除
settings-library-exclude-intro = 媒体库在上面所有来源里跳过的文件和文件夹。像 *.tmp 这样的名称在任何位置都匹配，像 Live/** 这样的路径从每个文件夹的顶层算起。不区分大小写，匹配的文件夹整个跳过。文件本身不动
settings-library-exclude-invalid = 读不懂这个模式：{ $reason }
settings-library-exclude-none = 没有排除项
settings-library-exclude-nudge = 模式已改：监视立刻跟上。已在媒体库里的曲目会留着，直到你按上面“来源”标题里的“重新扫描”
settings-library-exclude-placeholder = *.tmp 或 Live/**
settings-library-folder-col-albums = 专辑
settings-library-folder-col-size = 大小
settings-library-folder-col-tracks = 曲目
settings-library-folders-intro = 媒体库读取的文件夹和服务器；移除一个会把它的曲目从目录里去掉，文件夹里的文件不动
settings-library-genre-separator-nudge = 分隔符已改：浏览立刻跟上。早先扫描存下的流派列表会保持原样，直到你按上面“来源”标题里的“重新扫描”
settings-library-merge-case = 合并大小写变体
    .description = 只有大小写不同的值算作同一个：Rock 和 rock 变成同一个流派、艺术家和专辑，按多数曲目的写法显示。文件里的标签保持原样
settings-library-no-sources = 还没有来源
...
settings-library-section-sources = 来源
...
settings-playback-live-buffer-cap = 无论设成多长，缓冲都会停在 { $cap }，也就是这台机器内存的十分之一。
...
settings-playback-capture-buffer-title = 直播缓冲比大多数歌曲还短
settings-playback-capture-buffer-note = 现在是 { $buffer }，而一首歌只有在结束时仍完整留在缓冲里才会被保存。{ $default } 基本能装下电台放的任何东西。
...
settings-confirm-remove-subsonic-body = 登录会被忘记，它的曲目会离开媒体库。想要找回，就得重新连接并同步；关闭它则会保留这些曲目。
settings-confirm-remove-subsonic-title = 移除“{ $name }”？
...
tags-editor-guess-help = / 对应上一层文件夹，%skip% 丢弃
...
tags-rename-pattern-help = / 从音乐库根目录建文件夹，没有 / 则原地重命名，扩展名沿用原文件的
...
convert-dialog-pattern-help = / 建一层文件夹，扩展名由格式决定
...
lyrics-bold = 粗体
lyrics-build-word-by-word = 跟着唱到的词走
    .description = 让读头在当前行上走；歌词自带逐词时间时就按它走
...
lyrics-head-build = 逐词累加
lyrics-head-fill = 填充
lyrics-hide-upcoming = 隐藏待唱的行
    .description = 把播放位置还没走到的行收起来，让歌词随着播放逐步显现
...
lyrics-read-head-style = 读头
    .description = 当前行怎么展现读头：横扫过去的卡拉 OK 填充，还是轮到哪个词哪个词累加
...
lyrics-unsung-dim = 未唱部分变暗
    .description = 填充把当前行还没唱到的部分压暗多少
lyrics-wipe-lyrics = 清除歌词
lyrics-wrap-lines = 长行换行
    .description = 面板装不下的行换到下一行接着排，而不是裁掉
