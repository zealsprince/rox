### 简体中文。与 en-CA/rox.ftl 逐键对应；rox-i18n 里的
### 一致性测试盯着这一点。
...
align-row = 对齐
    .description = 面板有富余空间时，内容停在哪里
valign-row = 垂直对齐
    .description = 面板有富余高度时，内容停在哪里
valign-top = 顶部
valign-middle = 居中
valign-bottom = 底部
...
signal-threshold-gate = 低于此值信号读作零，高于它输出重新从零往上爬，安静的段落就推不动旋钮。上方表盘上的刻度就是它所在的位置
signal-low-bound = 低端
signal-high-bound = 高端
signal-adds-up = 累加对象
    .description = 这里累加哪个信号；那个信号读数高时它往上爬，安静时它停住
signal-aggregate-nothing = 没有可跟随的信号
signal-aggregate-pick = 选一个信号
signal-aggregate-alone = 信号池里没有别的信号可以累加，所以它停在零。加一个，它就会出现在列表里。
signal-aggregate-unpicked = 什么都没选，所以这个累加停在零。在上面选一个信号。
signal-rate = 速率
    .description = 满输入时每秒绕多少圈；过 1 就回到 0 继续爬，着色器把它读作相位
signal-reset-on-track = 换曲时重置
    .description = 新歌开始时排空回零，相位就不会把上一首的累加带进来
signal-flush = 清空
...
route-quiet = 安静
    .description = 静音时旋钮到达的位置，按它自身设定值的比例算
route-loud = 响亮
    .description = 满信号时它到达的位置；100% 就是滑块自身的值，低于“安静”则向下调制
route-slot = 槽位
...
panel-size-off = 关
panel-locked = 锁定
    .description = 把面板钉在原地；停靠区不会让它被拖动或重排
...
panel-own-opacity = 自定表面不透明度
    .description = 让这个面板在背景之上用自己的不透明度，而不是跟着应用走
...
panel-run-when-idle = 空闲时继续运行
    .description = 音频静默时也继续画帧。关掉后着色器就地停住，面板不花任何开销
...
bake-nothing-to-embed = 没什么可嵌入：文件里已经带着 rox 手上的全部内容
bake-rewrites = { $count ->
   *[other] 将重写 { $count } 个文件
}
bake-hint-before = 按
...
shader-blurb-badge = 封面缩成一张停在角落的小卡片，配一个槽位让它走来走去。
...
shader-note-file = { $path }。你保存的改动会在着色器绘制时热加载，源码也存在布局和包里，所以换到从没有这个文件的机器上它也还在。
...
library-headers = 分组标题
    .description = 列表上的分组断点；排序会让同属一组的连排留在一起，搜索时平铺显示
library-group-by = 分组依据
    .description = 分组标题按什么断开；流派和年份会重排列表
library-header-row = 标题行
    .description = 单行标题从左到右塞了什么；间隔或分隔线把两侧分开
...
library-row-height = 行高
    .description = 曲目行；文字跟着走，两者都随应用字体缩放
...
settings-appearance-design-mode = 设计模式
    .description = 就地编辑布局：面板菜单里的添加、重命名、复制、弹出和关闭各项，容器浮在槽位上的控件，以及标签页拖动。关掉后这些全都藏起来；“工作区”页仍然能编辑那棵树
...
settings-appearance-os-decorations = 系统窗口装饰
    .description = 主窗口上的系统标题栏和边框；关掉后就靠窗口按钮和拖动锚点面板
...
settings-audio-crossfade = 交叉淡化
    .description = 一首曲目和下一首重叠多久。淡化是给随机播放和跳曲用的，所以除非下面那行另有说明，专辑自身的曲目边界不受影响。为零就是关掉
...
settings-audio-output-experimental-tooltip = 这个平台的独占后端是照着平台文档里的音频约定写的，但开发者从没在真实硬件上跑过。它应该拿到设备，或者带着理由回退到共享，而不该没声音。要是它不听话，关掉它，用这个标记旁边的按钮报告发生了什么。
...
settings-audio-replaygain-measure-new = 测量新文件
    .description = 同步稳定之后，监视器带进来什么就测什么，媒体库越长越大也不用再回这里一趟。数字落在“保存测得增益”指向的地方。打开这一项时会先问要不要把已经缺失的补上；之后它只看刚刚落地的文件
...
settings-audio-replaygain-preamp = 前置增益
    .description = 加在每个标签增益上。ReplayGain 的参考值低于现在唱片的制作电平，所以做过电平均衡的媒体库听起来比原始的轻；这里就是把它补回来。提升不会削波：标签里的峰值会给它封顶
...
settings-audio-replaygain-status-tagged = 扫描到的 { $total } 首曲目全都带着 ReplayGain 标签
settings-audio-replaygain-untagged = 无标签文件
    .description = 没有 ReplayGain 标签的文件按什么电平播。没人测过它，所以这只是个顶替用的猜测，留在零，无标签的曲目就照旧播
...
settings-library-acoustic-auto = 描述新文件
    .description = 同步稳定之后，监视器带进来什么就描述什么，媒体库越长越大也不用再回这里一趟。关掉则新文件等着“分析缺失项”按钮。打开这一项时会先问要不要把已经缺失的补上；之后它只看刚刚落地的文件
...
settings-library-acoustic-save = 保存描述
    .description = 这一轮算出来的东西放在哪里。只用数据库不动你的文件；标签则在每个文件里也放一份，媒体库重建或文件夹换到别的机器上时描述还在，代价是重写音频文件。标签只到得了 MP3 和 FLAC，其他格式一律只留数据库那份
...
settings-library-tempo-auto = 给新文件测速
    .description = 同步稳定之后，监视器带进来什么就数它的拍子，媒体库越长越大也不用再回这里一趟。关掉则新文件等着“分析缺失项”按钮。打开这一项时会先问要不要把已经缺失的补上；之后它只看刚刚落地的文件
...
settings-library-tempo-status-tagged = 扫描到的 { $total } 首曲目全都带着速度标签
settings-library-watch-folders = 监视文件夹
    .description = 文件的添加、修改和删除随时并进媒体库，不用手动重新扫描
settings-library-write-stored = 把已存储的内容写进文件
    .description = 三个保存设置只对下一次写入生效，所以在切到“标签”之前保存的东西还只在 rox 里。这一步会把 rox 手上已有的歌词、增益和描述写进文件本身，交给别的播放器的文件夹也就带着它们。不会重新计算任何内容
...
settings-mcp-enable = 启用 MCP 服务器
    .description = 响应已连接 MCP 客户端的工具调用。代理每次调用都会查这一项，所以关着时客户端会被带着理由拒绝；下面的配置无论开关都能设好

## Settings: ML models
settings-mlmodels-checking = 正在检查…
settings-mlmodels-choose-file = 选择文件
settings-mlmodels-custom-description-empty = 给 rox 指一个你自己的 PANNs CNN10 检查点，safetensors 格式。它就地读取并按哈希命名，所以第二个检查点会单独描述媒体库，而不是落进第一个的坐标系里
...
settings-playback-play-order = 播放顺序
    .description = 随机开着时已入队的曲目怎么排。播放控制上的随机按钮负责开关；这里决定开了之后做什么
...
settings-providers-save-lyrics = 保存抓取的歌词
    .description = 抓来的歌词落在哪里：rox 自己的数据文件夹（媒体库保持干净）、曲目旁边的 .lrc，或者内嵌标签
...
settings-shader-backdrop-run-idle = 空闲时运行
    .description = 什么都没播时也继续画。动画无论如何都停在原处
settings-shader-compile-error-title = 这个着色器没编译过
settings-shader-legacy-note = 没有任何路由时，信号池按自己的顺序喂各个槽位：第一个信号进槽位 0，第二个进槽位 1，以此类推。你加的第一条路由会接管整个供给。
...
settings-shader-screen-run-idle = 空闲时运行
    .description = 什么都没播时也继续画。动画无论如何都停在原处。会读鼠标的着色器不开这一项也能在音乐停下时跟着光标走；只是指针停下两秒左右后它也停
...
settings-storage-looks-layouts = 外观和布局
    .description = 应用正穿着的这身外观（workspace.json），连同你保存的工作区、导出的着色器文件和图标包。很小，而且每一个字节都是你亲手设的
settings-storage-lyrics = 歌词
    .description = 抓来的和编辑过的歌词存在应用自己的库里（lyrics/），媒体库文件夹就保持干净
settings-storage-measured-tempos = 测得的速度
    .description = rox 从音频里数出来的速度，用于标签里没写的曲目；标签自身的数字不动。清掉会把那些曲目放回“媒体库”页“分析缺失项”的名单里，改进过的节拍检测就是这样覆盖旧结果的
...
settings-storage-models-empty = 模型
    .description = 还没有东西描述过媒体库。在“媒体库”页打开声学分析才会填上这里，跑过的每个模型都会占一行
...
settings-workspace-card-website-placeholder = 它住在哪儿
settings-workspace-composition-closed = 工作区窗口已关闭
settings-workspace-composition-hint = 窗口里的面板在分隔和标签组里的样子；箭头在同级里重排，锁把面板钉住，齿轮打开它的设置
settings-workspace-empty = 还没有工作区
settings-workspace-hint = 一个工作区就是一整套外观：布局、调色板、外观设置；应用一个会把三样全换掉
...
settings-confirm-clear-embeddings-body = 描述会消失，空间会回来。想再要一次，就得让分析把媒体库里每首曲目重新听一遍。
settings-confirm-clear-embeddings-title = 清除“{ $model }”描述的内容？
settings-confirm-clear-measured-bpm-body = rox 算出来的每个速度都回到未测状态；来自你文件自身标签的数字保留。想再要一次，就得让速度分析把那些曲目重新解码一遍。
...
workspace-import = 导入
workspace-launcher-hint = 加上第一个面板开始搭；或者在“工作区 > 应用工作区”里挑一个预设
...
tasks-scan-no-folders = 还没添加文件夹。从“文件”菜单打开一个
...
stats-count-menu = 计数
    .description = 这个数字统计的是哪一段时间内的收听；悬停列表里始终全都有
...
welcome-stage-lead-quick-start = 挑一个工作区，主窗口就会穿上它：布局、调色板、整套外观。
...
welcome-tour-intro = 快速看一遍音乐从哪儿进来、外观住在哪儿。最后停在随附工作区的架子上，一个一点。
...
signals-blurb-shared = 在这里调的东西是共享的：一处改动会落到路由到那个信号的每个参数上，跨面板跨窗口都算。
signals-blurb-total = 累加是第四种：它把另一个信号随时间加起来，到 1 就回绕，所以音乐响时它往上爬，不响时它停住。着色器需要一个跟着歌走而不是跟着时钟走的相位时，就用它。
...
eq-analyzer-wave = 波形
eq-band-badge = 频段角标
    .description = 在图标上的角标里数出偏离平直的频段数
...
eq-gain-label = 增益
eq-heading = 均衡器
eq-help-text = 拖动某个频段来移动它，在它上面滚动可以变宽或变窄。处理位于喂给声卡的缓冲之前，所以一次调整最多要半秒才到音箱。
eq-hint-off = 点击关闭
eq-hint-on = 点击开启
...
keymap-toggle-post-shader = 切换覆盖着色器
    .description = 关掉和打开屏幕着色器。到处都绑着是故意的：着色器可能把本来用来点它的每一个控件都埋掉
...
tags-rename-pattern-help = { $placeholders }；/ 建一层文件夹，扩展名跟着文件走
...
convert-dialog-checking = 正在问 ffmpeg…
...
convert-dialog-custom-note = 参数按空格拆分，所以不能加引号；自定义格式不带内嵌封面
...
convert-version-answered = { $binary } 有响应
...
cover-disc-style = 碟片样式
    .description = 把图片打扮成 CD，或者黑胶唱片的标签
...
lyrics-build-word-by-word = 逐词显现
    .description = 唱到哪个词就显出哪个词，卡拉 OK 式；没唱到的行先藏着
...
pass-acoustic-lands-tags = 结果进媒体库数据库，MP3 和 FLAC 还会进各自文件的标签，数据库重建后它们也还在。其他格式只留数据库那份。
...
drawer-open-on = 打开方式
    .description = 停在把手上总能打开抽屉；“选中”再加上主面板里的一次选中
...
particles-emitters-empty = 还没有发射器，加一个把这个场跑起来。
particles-glow = 辉光
...
shader-panel-note-off-body = 源码和它的绑定都还在，停着而已。
shader-panel-note-off-title = 这个着色器已关闭。
shader-panel-note-pending-body = 它是随布局或工作区来的，不是这台机器上的，所以在你看过之前先停着。
...
metadata-fields = 字段
    .description = 信息卡列出哪些字段；曲目没有的字段会藏起来
...
history-headings = 把最近列表按专辑连排断开；“展开”还会加上封面和统计
...
playlists-headings = 把每个播放列表的曲目按专辑连排断开；“展开”还会加上封面和统计
...
queue-clear = 清空队列
queue-empty = 队列是空的
queue-headings = 把队列按专辑连排断开；“展开”还会加上封面和统计
queue-play-now = 立即播放
queue-remove = { $count ->
   *[other] 从队列移除 { $count } 首
}
...
output-file-rate = 文件采样率
    .description = 没有任何转换时也确认一下播放文件自身的采样率。有转换时无论如何都会说明，因为警告说的就是这件事
...
track-info-delay = 延迟
    .description = 这行文字在两端各停多久才继续走
track-info-marquee = 跑马灯
    .description = 一行文字超出面板宽度时怎么办：爬过去再回来，还是无尽循环
...
## Volume panel
volume-pieces = 组件
    .description = 沿着这条栏拖动可以重排；在两行之间拖动，或者用小标签上的 x 和 +，来隐藏和显示。百分比藏起来时，喇叭的悬停提示会带着它
volume-readout = 读数
...
tasks-skipped-suffix = （跳过 { $count } 个）
...
workspace-shipped-diffuse-blurb = 只有正在播放的专辑：封面和播放卡片作为一组填满窗口，透明表面浮在背景上，没有接缝。媒体库、队列和歌词在右边缘的抽屉里候着，把手一悬停就滑出来盖在音乐上。刻意的单色：颜色交给封面。
