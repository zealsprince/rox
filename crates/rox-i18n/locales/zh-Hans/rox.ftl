...
stations-checking = 正在检查音频流...
stations-is-hls = 这是 HLS 分片列表，rox 播放的是一条连续的流
stations-is-a-playlist = 这是电台列表，请用导入电台读取它
stations-not-audio = 该地址返回的是 { $kind }，不是音频
stations-added-unchecked = 已添加但未检查：音频流没有响应（{ $reason }）
...
settings-playback-capture-pattern-source = %source% 是流的来源类型，新的来源类型会保存到各自的文件夹中。
...
settings-playback-section-streaming = 流媒体
...
autoeq-save = 保存
autoeq-saved = 已保存
autoeq-saved-info = 已保存为“{ $name }”
autoeq-save-failed = 无法写入预设文件
autoeq-applied-info = 已应用：{ $name }（{ $source }）
autoeq-applied-info-preamp = 已应用：{ $name }（{ $source }），前置增益 { $db } dB
autoeq-imported-info = 已导入：{ $name }
autoeq-imported-info-preamp = 已导入：{ $name }，前置增益 { $db } dB
autoeq-read-failed = 无法打开文件：{ $reason }
autoeq-parse-failed = 无法读取配置：{ $reason }
autoeq-import-untitled = 自定义
eq-flatten = 拉平
eq-freq-label = 频率
eq-gain-label = 增益
eq-heading = 均衡器
...
keymap-cue-next = 下一个标记
    .description = 跳到下一个标记
keymap-cue-clear = 清除标记
    .description = 移除正在播放曲目的所有会话标记
...
lastfm-import-history = 正在读取 scrobble 历史
...
player-stream-refused = 已跳过曲目：{ $reason }
