...
//! batched transactions like a real scan would produce; the read path is the
...
pub fn populate(conn: &mut Connection, seed: u64, total: u64) -> rusqlite::Result<()> {
    const BATCH: u64 = 50_000;
    let mut batch: Vec<TrackRow> = Vec::with_capacity(BATCH as usize);
    let flush = |conn: &mut Connection, batch: &mut Vec<TrackRow>| -> rusqlite::Result<()> {
    ...
    gen::generate(seed, total, |row| {
        batch.push(row);
        if batch.len() as u64 == BATCH && pending.is_ok() {
            pending = flush(conn, &mut batch);
        }
    });
    pending?;
...
/// to SQLite. Timed in the harness for contrast.
pub fn like_search(conn: &Connection, needle: &str) -> rusqlite::Result<u64> {
