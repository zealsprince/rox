...
/// The tile size knob's range, in px, the album grid's scale. The top
/// stays at the stored thumbnail's long side, so no setting upscales past
/// what either store keeps.
...
impl ArtistGridPanel {
    ...
    /// The caption's height when names are on, else zero.
    ...
    /// stack captions along the packing axis.
    ...
    /// A tile's resting opacity under the dim mode.
