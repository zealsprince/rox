...
- Scale envelope: the resident projection is measured to 10M tracks, worst-case search
  ...
  browse/search contract, which keeps that swap invisible to the UI.
...
- Real-time audio: the output callback is memcpy-only. Ring buffers are pre-allocated. This
  is a hard invariant: a lock or allocation there is an audible glitch.
- Tag writes: non-atomic at the lofty layer, so the metadata writer's copy-verify-rename is
  ...
  batch.
- Filesystem watching: `notify` isn't fully reliable, it drops events under load and can
...
- macOS is the solid target, Windows is acceptable with testing, Linux/Wayland is where the
  bug budget goes, especially the multi-window pop-out, which depends most on the platform
...
parts of the audio stack) that will break across upgrades. It's a standing line item, not
a one-time setup.
