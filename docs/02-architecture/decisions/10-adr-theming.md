...
Three writers go through that one setter. User edits, made in a settings window and
persisted in the settings file. A transparency pair of scalars, surface opacity and
backdrop strength, applied inside the background accessors rather than stored per token.
And a derived mode that, while a track plays, re-tints each token's hue and chroma toward
a seed color extracted from the cover art while preserving the token's lightness, so the
palette's contrast ladder is preserved for any album. Palette changes ease componentwise
from current to target; editing always targets the user palette, with derivation layered
on top. The backdrop is the playing track's art, downscaled and gaussian-blurred once per
track change on the existing background art path, resolved by a shared now-playing-art
entity and handed to gpui as a `RenderImage`; the bilinear upscale to window size
multiplies the blur. With several windows playing different tracks, the window that most
recently started one owns the seed and the backdrop is per window.
...
is outside gpui's reactivity, so repaints are explicit, which is fine at the rate
