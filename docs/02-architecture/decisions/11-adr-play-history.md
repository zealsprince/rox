...
Trade: counters give you "most played" and nothing else. Every question someone asks
...
for rollups and inherits the WAL durability story the store already has; a separate file
adds a second consistency story and buys nothing, and a log file outside SQLite gives
up the query path the rollups are built on.
...
([ADR 5](05-adr-library-store.md)), so track identity persists across a rescan and its
