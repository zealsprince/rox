...
Decision: the sizes, radii, and paces panels share are plain consts in a
`tokens` module beside the palette, both under one `design` module in the app
crate. Layout tokens are `Pixels` and go straight into div chains; paint tokens
...
bar rhythm the waveform and spectrum share. A value used by one control in
...
platform-native while rox is bespoke, so we take the idea and not the
dependency. Consts instead of palette-style runtime data mean no live
...
covered, the same source as layout.
