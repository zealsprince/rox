# ADR 23: Shader programs become pass chains, assets ride the bundle
...
scale, or stamp an image plate into a scene. All three want the same two things,
...
carry their metadata as comment directives. The cost is that pass boundaries live in
comments rather than types, caught at registration rather than deserialization,
which is where shader errors already land and already have a readout.
...
pragmatism as sixteen slots: past that, the design being expressed wants a render
...
carrying plates stay small in practice because the aesthetic that wants plates is
...
refused, and the product question underneath was answered rather than dodged:
rox's performance ambition is VJ-lite, live control and look-switching over
surfaces it already has, and the rendering ceiling that direction needs is chains
under a fixed compositor (an A/B blend between two chains, when that gets built),
never user-authored topologies. Everything else on the VJ-lite path is
control-plane work over this contract: hand-set slots and routes are the
performance knobs, ADR 22's socket is where a MIDI or OSC bridge lands, and a
popped-out Shader panel is the projector output. User-authored topology is
TouchDesigner's product, and even gig-grade VJ software is layers of linear chains
under a fixed compositor, which is how far the chain model demonstrably carries.
Passes with named inputs and outputs remain node-shaped regardless, so if this
read is ever revisited a chain lifts into a graph without breaking a bundle.
Compute passes are the honest answer for large sorts and were deferred on
capability grounds, since the gpui patches ride blade's render pipelines and a
compute stage is a different tier of surgery; a fragment chain covers the visible
aesthetic, and the pass contract here is likewise the one a compute stage would
